...
/**
 * Threads — flowing WebGL line field (from React Bits, adapted):
 * - TypeScript + Tailwind (no separate CSS file)
 * - mouse listeners on window so the container can stay pointer-events-none
 *   (it's a background layer; content above must remain clickable)
 * - prefers-reduced-motion renders a single static frame instead of looping
 * - render resolution capped + paused while off-screen / tab hidden
 */
...
export default function Threads({
  ...
  // Latest props live in a ref so changes mutate the live shader uniforms
  ...
  useEffect(() => {
    ...
    // Heavy per-pixel shader — cap internal render resolution.
    ...
    // Window-level listeners: the container is pointer-events-none (it's a
    // background), so it never receives mouse events itself.
    ...
    if (reduced) {
      // Single static frame — lines visible, no motion, no RAF loop.
      renderFrame(12000);
    } else {
      animationFrameId.current = requestAnimationFrame(update);
    }
